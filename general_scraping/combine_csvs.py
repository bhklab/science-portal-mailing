...
df_selected = combined_df[selected_columns].rename(
    columns={
        "Date": "date",
        "Title": "name",
        "Journal": "journal",
...
df_selected["PMID"] = (
    pd.to_numeric(df_selected["PMID"], errors="coerce").fillna(-1).astype(int)
)
...
filtered_selected_df = df_selected[
    ...
    &
    # (~df_selected['type'].str.contains('Early Access', case=False, na=False)) & # Had to be removed because some pubs don't get retroactively changed from Early Access to stricly Article
    (~df_selected["type"].str.contains("Meeting Abstract", case=False, na=False))
    & (
        df_selected["affiliations"].str.contains(
            "Princess Margaret", case=False, na=False
        )
        | df_selected["affiliations"].str.contains("PMC", case=False, na=False)
        | df_selected["filteredAffiliations"].str.contains(
            "Princess Margaret", case=False, na=False
        )
