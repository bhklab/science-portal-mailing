...
async def publication_scraping():
    df = pd.read_csv(
        f"{os.getcwd()}/general_scraping/mcgill/input_data/mcgill_publications.csv",
        encoding="utf-8",
    )
    df["PMID"] = pd.to_numeric(df["PMID"], errors="coerce").fillna(-1).astype(int)
    df = df.fillna("")
    ...
    with open(
        f"{os.getcwd()}/general_scraping/mcgill/errors/dois-of-errors.json",
        mode="w",
        encoding="utf-8",
    ) as f:
